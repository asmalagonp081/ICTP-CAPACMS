...
int main() {
    ...
    runManager->SetUserInitialization(new DetectorConstruction); // Geometría
    runManager->SetUserInitialization(new PhysicsList); // La fisica
    runManager->SetUserAction(new PrimaryGenerator); // La cinematica del evento generado
    runManager->Initialize(); // Inicialización

    G4UserRunAction* run_action = new UserDefinedRunAction;
    runManager->SetUserAction(run_action); // Acción del run definida por el usuario
